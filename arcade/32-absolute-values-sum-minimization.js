...
function absoluteValuesSumMinimization(a) {
  const getAbsDiffSum = (val, array) => {
    return array.reduce((sum, curr) => {
      return sum + Math.abs(val - curr);
    }, 0);
  }
  ...
  a.forEach(num => {
    const currentSum = getAbsDiffSum(num, a);
    if (currentSum < lowestSum) {
      lowestSum = currentSum;
      numOfLowestSum = num;
    }
  });
  return numOfLowestSum;
}



// Submitted successfully, passed 19 tests.
